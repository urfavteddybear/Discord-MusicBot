  ...
  run: async (client, interaction) => {
    if (
      ...
      try {
        ...
        client.bot.error(`Error executing command ${interaction.commandName}: ${error.message}`);
        console.error(error);
        
        // Log error to Discord
        await client.logErrorToDiscord(
          error,
          `Command Error: /${interaction.commandName} in ${interaction.guild.name} (${interaction.guild.id})`
        );
